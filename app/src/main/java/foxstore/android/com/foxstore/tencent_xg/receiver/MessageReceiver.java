package foxstore.android.com.foxstore.tencent_xg.receiver;
...
public class MessageReceiver extends XGPushBaseReceiver {
    ...
    public void onTextMessage(Context context, XGPushTextMessage message) {
        ...
        if (customContent != null && customContent.length() != 0) {
            try {
                ...
                if (!obj.isNull("key")) {
                    String value = obj.getString("key");
                    XGNotification notific= makeXGNotificationForGson(value);
                    NotificationService.getInstance(context).save(notific);
                    EventBus.getDefault().post(value,EventKes.ORDER_PUBLISH);
                    Log.d(LogTag, "get custom value:" + value);
                }
                // ...
            } catch (JSONException e) {
    ...
    //制作透传通知
    private XGNotification makeXGNotificationForGson( String gson) {

        XGNotification notific = new XGNotification();

        Order order= GsonUtil.json2Object(gson,new TypeToken<Order>(){});

        if(order!=null){
            notific.setMsg_id(order.getObjectId());
        notific.setTitle(gson);
        notific.setContent(gson);
        // notificationActionType==1为Activity，2为url，3为intent
        notific.setNotificationActionType(Message.TYPE_MESSAGE);
        //Activity,url,intent都可以通过getActivity()获得
//        notific.setActivity(notifiShowedRlt.getActivity());
        notific.setUpdate_time(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")
                .format(Calendar.getInstance().getTime()));
